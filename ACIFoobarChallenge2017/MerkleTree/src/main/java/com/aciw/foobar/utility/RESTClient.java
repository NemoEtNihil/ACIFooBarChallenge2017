...
public class RESTClient {
	 ...
	 * 
	 * @param answer The hash you believe is contained within the root node of the Merkle Tree.
	 * @param setId Chooses the challenge set you wish to solve. 
	 * 				setId=1 for main challenge set.
	 * 				setId=2 for bonus challenge set.
	 */
	public static void sendOutput(String answer, int setId) {
		try {
			// prep the client
			Client client = ClientBuilder.newClient();
			String baseUrl = retrieveUrl();
			WebTarget webTarget = client.target(baseUrl + "challenges/merkleTree/submit");

			// create object to send out in POST
			AnswerMessenger msg = new AnswerMessenger(answer, setId);
	...
	public static ArrayList<Transaction> getInput(int set) {
		...
		// service.
		String baseUrl = retrieveUrl();
		WebTarget webTarget = client.target(baseUrl + "challenges/merkleTree/" + set);
		String authValue = Base64.getEncoder().encodeToString((username + ":" + password).getBytes());
		Invocation.Builder invocationBuilder = webTarget.request(MediaType.APPLICATION_JSON).header("Authorization",
				"Basic " + authValue);
		...
		if (checkLoginPassed(rspStr)) {
			...
			ArrayList<Transaction> result = response.readEntity(new GenericType<ArrayList<Transaction>>() {
			});
			return result;
		} else {
			// Login failed
