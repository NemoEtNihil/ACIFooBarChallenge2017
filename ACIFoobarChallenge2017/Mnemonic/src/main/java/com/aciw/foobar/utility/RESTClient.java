...
public class RESTClient {
	 ...
	 * 
	 * @param answer The list of words which you believe makes up the mnemonic.
	 * @param setId Chooses the challenge set you wish to solve. 
	 * 				setId=1 for main challenge set.
	 * 				setId=2 for bonus challenge set.
	 */
	public static void sendOutput(List<String> answer, int setId) {
		try {
			// prep the client
			Client client = ClientBuilder.newClient();
			String baseUrl = retrieveUrl();
			WebTarget webTarget = client.target(baseUrl + "challenges/Mnemonics/submit");

			// create object to send out in POST
			AnswerMessenger msg = new AnswerMessenger();
