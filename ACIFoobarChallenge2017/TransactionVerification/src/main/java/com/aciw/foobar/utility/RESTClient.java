...
public class RESTClient {
	 ...
	 * 
	 * @param sourcePublicKey Owner 0's public key in its original form. (Not hashed) 
	 * @param sourceSignature Owner 0's signature used in sending the original transaction.
	 */
	public static void sendOutput(String sourcePublicKey, String sourceSignature) {
		try {
			// only 1 set for this challenge.
			int setId = 1;

			// prep the client
			Client client = ClientBuilder.newClient();
			String baseUrl = retrieveUrl();
			WebTarget webTarget = client.target(baseUrl + "challenges/TransactionVerification/submit");
			...
			AnswerMessenger msg = new AnswerMessenger(sourcePublicKey, sourceSignature, setId);
